...
while(i < N){
        ...
        //reste du code sans utilisation du mute
        i++;
}
}
...
pthread_attr_t attr;
pthread_mutexattr_t m_attr; //La déclaration de la propriété à affecter au mutex d'exclusion mutuelle
struct sched_param param;

/*préparation de la propriété du verrou*/
// La fonction d'initialisation de la propriété du mutex. Obligatoire avant toute manipulation de l'attribut du mutex
pthread_mutexattr_init(&m_attr);
// La fonction permettant à un mutex d'être partagé par les tâches appartenant à n'importe quel processus.
pthread_mutexattr_setpshared(&m_attr, PTHREAD_PROCESS_SHARED);
pthread_mutexattr_setprioceiling(&m_attr, 15); //le mutex dispose d'une priorité de 15, quisera celle de la tâche la plus prioritaire
pthread_mutexattr_setprotocol(&m_attr, PTHREAD_PRIO_PROTECT); // permettant de mettre le protocole d'heritage
// dans ce cas chaque thread herite la priorité du verrou
...
pthread_attr_setscope(&attr, PTHREAD_SCOPE_SYSTEM); // La fonction permettant à une tâche disposant de la prorpiété attr d'hériter d'une priorité.
